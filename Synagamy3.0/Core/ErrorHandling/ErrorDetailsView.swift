...
struct ErrorDetailsView: View {
        ...
        NavigationView {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    // Recent Errors
                    if !errorHandler.getErrorHistory().isEmpty {
                        recentErrorsSection
                    }
    ...
    private var errorDetailsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let currentRecord = errorHandler.getErrorHistory().first {
                    DetailRow(title: "Context", value: currentRecord.context)
                }
            }
        }
        .padding(16)
    ...
    private var recentErrorsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVStack(spacing: 8) {
                ForEach(Array(errorHandler.getErrorHistory().prefix(5))) { record in
                    ErrorHistoryRow(record: record)
                }
            }

            if errorHandler.getErrorHistory().count > 5 {
                Text("... and \(errorHandler.getErrorHistory().count - 5) more")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .padding(.top, 4)
            }
    ...
    private func copyErrorToClipboard() {
        ...
        Recent Errors:
        \(errorHandler.getErrorHistory().prefix(3).map { "- \($0.error.userFriendlyTitle) at \(ErrorDetailsView.formatTimestamp($0.timestamp))" }.joined(separator: "\n"))
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(ErrorDetailsView.formatTimestamp(record.timestamp))
                    .font(.caption2)
                    .foregroundColor(.secondary)
            }
...
#Preview {
    ErrorDetailsView(
        error: .calculationFailed(details: "Invalid AMH value"),
