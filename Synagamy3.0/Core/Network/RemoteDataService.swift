...
final class RemoteDataService: ObservableObject {
    ...
    @Published private(set) var connectionStatus: RemoteDataConnectionStatus = .unknown
    @Published private(set) var lastError: String?
    
    private let secureNetworkManager = SecureNetworkManager.shared
    private let cacheManager = DataCacheManager.shared
    private let networkManager = NetworkStatusManager.shared
    ...
    enum RemoteDataConnectionStatus: Equatable {
        case unknown
        case connected
        case offline
        case error(String)
    ...
    private func loadData<T: Codable>(
        ...
        // Check network availability before attempting network operations
        if networkManager.networkStatus.canPerformNetworkOperations {
            #if DEBUG
            print("🌐 loadData: Network is available, attempting fetch...")
            #endif
            ...
            if let networkData = await fetchFromNetwork(type, from: urlString) {
                ...
                let offlineCategory = mapToOfflineCategory(type: type)
                await offlineManager.storeForOfflineUse(networkData, forKey: cacheKey, category: offlineCategory)
                ...
                #endif
                lastError = networkManager.getErrorMessage(for: "load \(cacheKey)")
            }
        } else {
            #if DEBUG
            print("⚠️ loadData: Network unavailable: \(networkManager.networkStatus.displayName)")
            #endif
            lastError = networkManager.getErrorMessage(for: "load \(cacheKey)")
        }
        ...
        // Try offline data storage before final fallback
        let offlineCategory = mapToOfflineCategory(type: type)
        if let offlineData = offlineManager.retrieveFromOfflineStorage(type, forKey: cacheKey, category: offlineCategory) {
            #if DEBUG
            print("✅ loadData: Successfully loaded from offline storage for \(cacheKey)")
            #endif
            offlineManager.setOfflineMode(true)
            return offlineData
        } else {
            #if DEBUG
            print("❌ loadData: No offline data available for \(cacheKey)")
            #endif
        }
    ...
    /// Map data types to offline storage categories
    private func mapToOfflineCategory<T>(type: T.Type) -> OfflineDataCategory {
        switch type {
        case is [EducationTopic].Type:
            return .education
        case is [CommonQuestion].Type:
            return .commonQuestions
        case is PathwayData.Type:
            return .pathways
    ...
    private func fetchFromNetwork<T: Codable>(_ type: T.Type, from urlString: String) async -> T? {
        ...
        for attempt in 1...Config.retryAttempts {
            ...
            do {
                // Use secure network manager for GitHub data fetching with certificate pinning
                let data = try await secureNetworkManager.secureDataFetch(from: url)

                #if DEBUG
                print("📦 fetchFromNetwork: Securely received \(data.count) bytes")
                #endif
                ...
                await updateConnectionStatus(RemoteDataConnectionStatus.connected)
                ...
                if attempt == Config.retryAttempts {
                    ...
                    #endif
                    await updateConnectionStatus(RemoteDataConnectionStatus.error(error.localizedDescription))
                } else {
                    #if DEBUG
    ...
    private func checkConnectivity() async {
        ...
        guard let url = URL(string: DataEndpoints.educationTopics) else {
            ...
            #endif
            await updateConnectionStatus(RemoteDataConnectionStatus.error("Invalid base URL"))
            return
        }
        ...
        do {
            // Test secure connection using the secure network manager
            if secureNetworkManager.validateConnectionSecurity(for: url) {
                // Test with a lightweight request to verify connectivity
                _ = try await secureNetworkManager.secureDataFetch(from: url)
                await updateConnectionStatus(RemoteDataConnectionStatus.connected)
                #if DEBUG
                print("✅ checkConnectivity: Secure connection successful")
                #endif
            } else {
                await updateConnectionStatus(RemoteDataConnectionStatus.offline)
                #if DEBUG
                print("⚠️ checkConnectivity: Connection not secure - marking as offline")
                #endif
            }
        } catch {
            #if DEBUG
    ...
    private func updateConnectionStatus(_ status: RemoteDataConnectionStatus) async {
        await MainActor.run {
